...
# Running inference
ANN-CLI --config <model_file> --mode run --input <input_file> [options]
...
| `--mode` | `-m` | Mode: `train`, `run`, or `test` (overrides config file) |
...
- **run**: Run inference using `--config` (trained model) with a single input.
...
For running inference with a trained model:
...
- `mode`: Operation mode (optional, default: `run`) — *can be overridden by `--mode`*
...
This file is generated by `--mode train` and can be used directly with `--config` for `--mode run` and `--mode test`.
...
```bash
ANN-CLI --config trained_model.json --mode run --device gpu --input test_input.json
```
