...
static void testMissingConfig()
{
  ...
  CHECK(result.stdErr.contains("Error: --config is required."), "Missing config: error message");
  std::cout << std::endl;
}
...
static void testInvalidDevice()
{
  ...
  auto result = runNNCLI({"--config", fixturePath("ann_train_config.json"), "--mode", "train", "--device", "tpu"});
...
static void testInvalidActvFuncCNN()
{
  ...
  auto result = runNNCLI({"--config", fixturePath("cnn_invalid_actvfunc_config.json"), "--mode", "train", "--device",
